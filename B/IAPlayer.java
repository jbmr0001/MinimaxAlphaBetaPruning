...
public class IAPlayer extends Player {
    ...
    public void generaHijos2(Estado e) {
        ...
        if (e.solucion) { //EN CASO DE QUERER MOSTRAR LOS ESTADOS GANADORES CONFORME SE GENERAN
            ...
            System.out.println("------------------------------");
             */        } else {
            if (e.estadoFinal == false && e.generacion < 5) {
                hijoEnColumna(e);
...
//Genera una columna en la que insertar la siguiente ficha
    public int MiniMax(Estado e) { 
        int valor = Max(e).getValue(); //Llamamos a la función Max con el estado actual y sacamos de este el valor del pair que es el indice del hijo que nos sirve
    ...
    public Pair<Double, Integer> Max(Estado e) {
        ...
        for (int i = 0; i < e.hijos.size(); i++) { //recorremos todos los hijos del estado e
            if (e.hijos.get(i).estadoFinal == true||e.hijos.get(i).generacion==5) { //en caso de ser un estado final o un estado con 5 generaciones
                valores.add(e.hijos.get(i).valor); //añadimos el valor a la lista de valores 
            } else { //en caso de ser un estado intermedio

                valores.add(Mini(e.hijos.get(i)).getKey()); //llamamos a la función mini con el hijo de i, haciendo que nos devuelva valor de la heuristica de este
            }
        }
...
//Devuelve un pair con el valor de la heuristica y la posición del estado hijo en la lista enlazada del padre
    public Pair<Double, Integer> Mini(Estado e) {
        LinkedList<Double> valores;//creamos una lista enlazada con los valores de la heuristica
        valores = new LinkedList<>();
        for (int i = 0; i < e.hijos.size(); i++) {//recorremos todos los hijos del estado e
            if (e.hijos.get(i).estadoFinal == true||e.hijos.get(i).generacion==5) {//en caso de ser un estado final o un estado con 5 generaciones
                ...
                valores.add(Max(e.hijos.get(i)).getKey());//llamamos a la función max con el hijo de i, haciendo que nos devuelva valor de la heuristica de este
            }
        }
